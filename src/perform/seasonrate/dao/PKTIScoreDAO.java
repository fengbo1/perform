...
public class PKTIScoreDAO extends BaseHibernateDAO  {
     ...
     * @return
     */
    public List<PKTIScore> findByYearSeasonRaterNotRate(int year,int season,String rater,String newnumber) {
    	log.debug("finding all PKTIScore instances");
    	try {
    		String queryString = "from PKTIScore where year='"+year+"' and season='"+season+"' and newnumber='"+newnumber+"' and ((rater1='"+rater+"' and result1='0') or (rater2='"+rater+"' and result2='0') or (rater3='"+rater+"' and result3='0'))";
             Query queryObject = getSession().createQuery(queryString);
    		 List<PKTIScore> list = queryObject.list();
    		 return list;
    	} catch (RuntimeException re) {
