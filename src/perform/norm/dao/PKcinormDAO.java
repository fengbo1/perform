 ...
 * PKcinorm entities. Transaction control of the save(), update() and delete()
 * operations can directly support Spring container-managed transactions or they
 * can be augmented to handle user-managed Spring transactions. Each of these
 * methods provides additional information for how to configure it for the
 * desired type of transaction control.
 * 
 * @see perform.norm.pojo.PKcinorm
 * @author MyEclipse Persistence Tools
 */

public class PKcinormDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory
			.getLogger(PKcinormDAO.class);
	// property constants
	...
	public PKcinorm findById(java.lang.Integer id) {
		...
		try {
			PKcinorm instance = (PKcinorm) getSession().get(
					"perform.norm.pojo.PKcinorm", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(PKcinorm instance) {
		...
		try {
			List results = getSession().createCriteria(
					"perform.norm.pojo.PKcinorm").add(Example.create(instance))
					.list();
	...
	public PKcinorm merge(PKcinorm detachedInstance) {
		...
		try {
			PKcinorm result = (PKcinorm) getSession().merge(detachedInstance);
	...
	public void attachClean(PKcinorm instance) {
	 ...
	 public PKcinorm findByLevelAndName(String level,String name) {
	    	log.debug("finding all PKcinorm instances");
	    	try {
	    		String queryString = "from PKcinorm as kc where kc.level='"+level+"'  and kc.name='"+name+"'";
	             Query queryObject = getSession().createQuery(queryString);
