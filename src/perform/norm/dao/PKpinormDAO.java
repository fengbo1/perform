 ...
 * PKpinorm entities. Transaction control of the save(), update() and delete()
 * operations can directly support Spring container-managed transactions or they
 * can be augmented to handle user-managed Spring transactions. Each of these
 * methods provides additional information for how to configure it for the
 * desired type of transaction control.
 * 
 * @see perform.norm.pojo.PKpinorm
 * @author MyEclipse Persistence Tools
 */

public class PKpinormDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory
			.getLogger(PKpinormDAO.class);
	// property constants
	...
	public PKpinorm findById(java.lang.Integer id) {
		...
		try {
			PKpinorm instance = (PKpinorm) getSession().get(
					"perform.norm.pojo.PKpinorm", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(PKpinorm instance) {
		...
		try {
			List results = getSession().createCriteria(
					"perform.norm.pojo.PKpinorm").add(Example.create(instance))
					.list();
	...
	public PKpinorm merge(PKpinorm detachedInstance) {
		...
		try {
			PKpinorm result = (PKpinorm) getSession().merge(detachedInstance);
