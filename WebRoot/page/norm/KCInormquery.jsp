...
brbrbr();
 ...
 function expressout()
	{
		window.location = "<%=path%>/exportkcinorm.action";
	}
 ...
 function del(id)
{
	...
	if (aa) {
		window.location = "<%=path%>/kcidel_mang.action?id="+id;
	}
}
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
  ${daohang}
   <form action="<%=path%>/kcilistquery.action" method="post" name="fm1">
					<table height="80" align="center" cellpadding="0"
						cellspacing="2" >
						<tr>
							<td
								style="color: #1778C2; padding-top: 15px; padding-bottom: 15px; border: 0px; font-size: 26px; font-family: '黑体';"
								colspan="12" align="center" bordercolor="#FFFFFF"><b>关键胜任力指标KCI指标库</b>
							</td>
						</tr>
						<tr>
						...
						<c:if test="${authoR=='R'}">	
							...
							<c:forEach items="${list}" var="kc" varStatus="status">
							...
							<div align="center">
								<a	href="<%=path%>/kcilistquery.action?level=${level}&currentPage=${previousPage}"
							style="padding-right: 30px;color: #104E8B">上一页</a> 
									${currentPage} of ${totalPages}
								<a	href="<%=path%>/kcilistquery.action?level=${level}&currentPage=${nextPage}"
							...
							<td>
							 ...
							 <input type="button" value="添加新指标" onclick="location='<%=path%>/page/norm/KCIadd_mang.jsp'"/>	
